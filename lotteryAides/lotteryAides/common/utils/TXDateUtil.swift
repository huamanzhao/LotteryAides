...
class TXDateUtil: NSObject {
    ...
    static func getTodayDateFromString(_ dateStr: String) -> Date {
        ...
        return TXDateUtil.getDateStart(Date()).addingTimeInterval(Double(timeInterval))
    }
...
//        return self == Date()
    ...
    static func getDisplayTime(_ timeInMillis: String) -> String {
        ...
        //今年之前的，返回年月日
        if distance > 365 {
            return date.toString(format: FORMAT_WHOLE_DATE_CN)
        }
        
        //今年的，前天之前
        if distance > 2 {
            return date.toString(format: FORMAT_MONTH_DAY_CN)
        }
        
        //前天
        if distance == 2 {
            return "前天 " + date.toString(format: FORMAT_TIME)
        }
        
        //昨天
        if distance == 1 {
            return "昨天 " + date.toString(format: FORMAT_TIME)
        }
        
        //今天
        if distance == 0 {
            return "今天 " + date.toString(format: FORMAT_TIME)
        }
        
        return date.toString(format: FORMAT_WHOLE_DATE)
    }
}
